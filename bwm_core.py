...
class WaterMarkCore:
    def __init__(self, password_img=1, mode='common', processes=None):
        ...
        self.d1, self.d2 = 22, 18  # d1/d2 越大鲁棒性越强,但输出图片的失真越大
    ...
    def init_block_index(self, noCheck = False):
        self.block_num = self.ca_block_shape[0] * self.ca_block_shape[1]
        if not noCheck:
            assert self.wm_size < self.block_num, IndexError(
                'Overflown because up to {}KB of data can be embedded, which is more' 
                'than the {}KB of data in the watermark.'.format(self.block_num / 1000, self.wm_size / 1000))
    ...
    def extract_raw(self, img):
        ...
        self.init_block_index(noCheck=True)
